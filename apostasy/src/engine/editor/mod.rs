use crate::{
    self as apostasy,
    engine::ecs::{World, entity::Entity},
};
use apostasy_macros::ui;
use egui::{
    CentralPanel, Color32, Context, CursorIcon, Frame, Id, Rect, Sense, SidePanel, Stroke,
...
pub fn editor_ui(context: &mut Context, world: &mut World) {
    ...
    CentralPanel::default()
        ...
        .show(context, |ui| {
            ...
            let vdiv_rect = Rect::from_min_max(
                pos2(left_panel_rect.max.x, main_rect.min.y),
                pos2(left_panel_rect.max.x + DIV, main_rect.max.y),
            );
            let right_rect =
                Rect::from_min_max(pos2(vdiv_rect.max.x, main_rect.min.y), main_rect.max);
            ...
            hierarchy_ui.painter().rect_filled(
                hierarchy_rect,
                0.0,
                Color32::from_rgb(230, 210, 40),
            );
            hierarchy_ui.label("Hierarchy");

            let hierarchy_resp = ui.allocate_rect(hierarchy_rect, Sense::drag());
            let hierarchy_color = if hierarchy_resp.hovered() || hierarchy_div_rect.dragged() {
            ...
            ui.painter()
                .rect_filled(hierarchy_rect, 0.0, hierarchy_color);
            ...
            let vdiv_resp = ui.allocate_rect(vdiv_rect, Sense::drag());
            let vdiv_color = if vdiv_resp.hovered() || vdiv_resp.dragged() {
                context.set_cursor_icon(CursorIcon::ResizeHorizontal);
                Color32::from_gray(180)
            } else {
                Color32::from_gray(60)
            };
            ui.painter().rect_filled(vdiv_rect, 0.0, vdiv_color);
            if vdiv_resp.dragged() {
                left_ratio =
                    (left_ratio + vdiv_resp.drag_delta().x / main_rect.width()).clamp(0.1, 0.5);
            }
