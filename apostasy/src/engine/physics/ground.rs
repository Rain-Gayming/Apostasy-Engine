...
pub fn update_ground_state(world: &mut World, delta_time: f32) {
        ...
        for node in all_nodes {
                ...
                && node.get_component::<Transform>().is_some()
            {
                nodes_to_check.push((
                    ...
                    node.get_component::<Transform>().unwrap().global_scale,
                ));
            }
        }
    }
    ...
    for (node_name, position, scale) in nodes_to_check {
        // Skip editor camera
        ...
        if let Some(node) = world.get_node_with_name_mut(&node_name) {
            ...
            if let Some(collider) = node.get_component::<Collider>() {
                let raycast_length = 1.1 * (collider.half_extents().y * scale.y);

                let raycast = Raycast::new(Vector3::new(0.0, -1.0, 0.0), raycast_length);

                let hits = raycast.cast_from(
                    position,
                    Vector3::new(0.0, -1.0, 0.0),
                    &node_refs,
                    &node_name, // filters out self-detection
                );
