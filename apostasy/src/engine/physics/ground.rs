...
pub fn update_ground_state(world: &mut World, delta_time: f32) {
    ...
    let mut nodes_to_check = Vec::new();
    {
        ...
        for node in all_nodes {
            if node.get_component::<Velocity>().is_some() && node.get_component::<Transform>().is_some() {
                nodes_to_check.push((node.name.clone(), node.get_component::<Transform>().unwrap().position));
            }
        }
    }
    ...
    for (node_name, position) in nodes_to_check {
        ...
        let raycast = Raycast::new(Vector3::new(0.0, -1.0, 0.0), 1.5);
