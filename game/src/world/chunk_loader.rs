...
pub fn update_chunks_init(world: &mut World) -> Result<()> {
    // Initialise noise statics BEFORE any generation jobs are dispatched.
    // dispatch_chunk_jobs only writes these when GetNewSeed is present, so on a
    // cold start they would be None when the first rayon jobs run, causing silent
    // panics on the worker threads and permanently empty chunks.
...
const MAX_GEN_JOBS_PER_FRAME: usize = 4;
#[fixed_update]
