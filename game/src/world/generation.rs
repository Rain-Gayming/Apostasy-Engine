...
fn sample_height_and_biome(
    ...
    // These statics must be initialised before this function is called.
    // generate_chunk_data already guards at its entry point, so by the time we
    // reach here they are guaranteed to be Some. Use expect so a logic error
    // produces a clear message rather than a silent wrong result.
    let temp_noise = TEMPERATURE_NOISE
        .read()
        .expect("TEMPERATURE_NOISE lock poisoned")
        .expect("TEMPERATURE_NOISE not initialised — noise statics must be set before generation");
    let humid_noise = HUMIDITY_NOISE
        .read()
        .expect("HUMIDITY_NOISE lock poisoned")
        .expect("HUMIDITY_NOISE not initialised");
    let continental_noise = CONTINENTAL_NOISE
        .read()
        .expect("CONTINENTAL_NOISE lock poisoned")
        .expect("CONTINENTAL_NOISE not initialised");
...
pub fn generate_chunk_data(
    ...
    // Guard: noise statics may not be initialised yet if a gen job races the
    // startup seed write (e.g. the #[start] job dispatches before GetNewSeed is
    // processed). Return an empty chunk so the slot stays in the channel and the
    // caller can detect it and re-queue if needed.
    let noise = match NOISE.read().ok().and_then(|g| *g) {
        Some(n) => n,
        None => return GeneratedChunkData::empty(position, lod),
    };
    ...
    let _continental_noise = CONTINENTAL_NOISE.read().unwrap().unwrap();
