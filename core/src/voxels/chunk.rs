...
impl GeneratedChunkData {
    /// Returns an all-air chunk at the given position. Used as a safe fallback
    /// when noise globals are not yet initialised (e.g. a job races the startup
    /// seed write). The chunk will be correctly re-queued once the seed is set.
    pub fn empty(position: Vector3<i32>, lod: u8) -> Self {
        Self {
            position,
            voxels: Box::new([0; 32 * 32 * 32]),
            lod,
            biome: 0,
        }
    }
