...
pub struct VulkanRenderer {
    ...
    pub buffers_to_destroy: Vec<(vk::Buffer, vk::DeviceMemory)>,
    ...
    fn begin_frame(&mut self, _push_constants: PushConstants) -> Result<()> {
        ...
        unsafe {
            ...
            match self.context.device.wait_for_fences(
                ...
                Ok(()) => {
           if !self.buffers_to_destroy.is_empty() {
    println!("Draining {} buffers from graveyard", self.buffers_to_destroy.len());
}
for (buffer, memory) in self.buffers_to_destroy.drain(..) {
    unsafe {
    ...
    fn get_destroy_buffers(&mut self) -> Vec<(vk::Buffer, vk::DeviceMemory)> {
        self.buffers_to_destroy.drain(..).collect()         
    }
